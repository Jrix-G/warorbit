...
RUN_NAME="${RUN_NAME:-gpu_2p_lightning_clean_10h}"
...
if [[ -f "$CURRENT_LATEST" ]]; then
  ...
  echo "Resuming clean run from $CURRENT_LATEST"
elif [[ -f "$SOURCE_BEST" ]]; then
  RESUME_ARGS=(--resume-checkpoint "$SOURCE_BEST")
  echo "Starting clean run from source best $SOURCE_BEST"
elif [[ -f "$SOURCE_LATEST" ]]; then
  RESUME_ARGS=(--resume-checkpoint "$SOURCE_LATEST")
  echo "Source best missing; starting clean run from source latest $SOURCE_LATEST"
else
  echo "No checkpoint found; starting clean run from scratch"
fi
...
python "$RUNNER_PATH" \
  ...
  --min-expand-attack-ships 6 \
  --send-ratios "0.45,0.60,0.75,0.90" \
  --simple-opponents "random,greedy,greedy,distance" \

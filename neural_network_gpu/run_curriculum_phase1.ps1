...
$RunName = "gpu_2p_curriculum_phase1_from_imitation"
...
state["metadata"] = json.dumps({
    ...
    "curriculum_phase": "phase1_random_greedy",
})
...
# Phase 1: easy enough to give PPO a clean signal, but still guarded against passive/no-ship exploits.
$TrainOpponents = "random,random,random,random,random,greedy,greedy,greedy,greedy,greedy"
...
$Args = @(
  ...
  # Anti-passivity shaping: legal no-op only. Forced no-ops should not be punished.
  ...
  "--per-step-real-action-bonus", "0.010",
  "--per-step-shape-clip", "0.04",

  # Raw no-op backstops stay loose because raw no-op includes forced no-ops.
  "--max-eval-do-nothing-rate", "0.86",
  "--valid-win-max-do-nothing-rate", "0.90",
  "--rollback-on-noop-rate", "0.0",

  # Hard anti-exploit gates: random/greedy wins with no ships must not validate.
  ...
  # Promotion/regression guard. Phase 1 monitors starter/distance but does not train on them.
...
if ($UseTeacher) {
  $Args += @("--teacher-checkpoint", $TeacherCheckpoint, "--teacher-kl-coef", "0.010")
} else {

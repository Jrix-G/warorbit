"""Orbit Wars V8 — persistent state machine + staged transfer economy.

V8.1 (2026-04-30) — conservative pass after first benchmark regressed (30% WR,
~76s/game, timeouts). Root causes:
  1. _execute_v8_transfers ran before follow-up captures, eating budget.
  2. Triple-staging (V8 transfer + V7 REAR staging + V7 anti-hoarding) drained
     rear planets and exploded fleet count -> games ~15x slower.
  3. staging-mode attack damping (0.88x) made bot hoard instead of attack.
  4. staging-mode triggered too early (n_my>=7, no step gate).
  5. Loose gates in staging/finish let any rear planet transfer, draining
     productive planets.
V8.1 keeps the mode machine + kovi-aligned finish trigger (>=14 planets,
<=4 enemy) but disables the destructive paths. V7's existing REAR staging
and anti-hoarding already cover transfer logistics; the V8 transfer pass
now only fires in finish mode with hard caps.
"""
...
# --- V8: persistent state machine and transfer-first logistics ---
V8_CONVERSION_T60_TARGET = 13
V8_CONVERSION_T100_TARGET = 16
V8_FINISH_PLANET_THRESHOLD = 14
V8_FINISH_ENEMY_PLANET_THRESHOLD = 4
V8_OPENING_NEUTRAL_BOOST = 1.06        # gentle; V7 already pulls neutrals well
V8_CONVERSION_NEUTRAL_BOOST = 1.10     # was 1.24 — too aggressive on top of V7
V8_STAGING_ATTACK_DAMPING = 1.00       # was 0.88 — damping caused hoarding regression
V8_FINISH_HOSTILE_BOOST = 1.18         # was 1.22 — small nudge, finish flag already exists
V8_FINISH_SEND_BONUS = 2               # was 3 — V7 already adds FINISHING_HOSTILE_SEND_BONUS
V8_STAGING_MIN_PLANETS = 10            # was 7 — staging only after real territorial lead
V8_STAGING_MIN_STEP = 60               # don't enter staging during opening/early-mid

# V8 transfer pass: V7 already has REAR staging (line ~1762) + anti-hoarding
# (line ~1802). The V8 transfer is now a *supplemental* pass, only in finish
# mode, with strict gates and a small move cap. Anything wider regressed.
V8_TRANSFER_ENABLED = True
V8_TRANSFER_FINISH_ONLY = True         # V8.1: only run in finish mode
V8_TRANSFER_MIN_SHIPS = 12             # was 6 — only push meaningful blobs
V8_TRANSFER_MIN_PROD_RATIO = 6
V8_TRANSFER_HOARD_PROD_RATIO = 12      # was 9 — leave more garrison
V8_TRANSFER_BACK_RATIO = 1.20          # was 1.12 — only deep-rear planets
V8_TRANSFER_STAGE_PROGRESS = 0.86
V8_TRANSFER_MAX_TRAVEL_TURNS = 45      # was 55 — avoid long detours
V8_TRANSFER_MAX_MOVES = 2              # was 8 — kovi median is 1-2 sources/turn
V8_TRANSFER_RATIO_FINISH = 0.55        # was 0.66
...
class V8Memory:
    ...
    mode: str = "opening"
    mode_since: int = 0
    front_anchor_id: object = None
    last_planet_count: int = 0
    last_production: int = 0


_V8_MEMORY_BY_PLAYER = {}
...
def _frontier_targets(world):
    return world.enemy_planets or world.static_neutral_planets or world.neutral_planets


def _frontier_distance_map(world):
    targets = _frontier_targets(world)
    if not targets:
...
def _get_v8_memory(world):
    mem = _V8_MEMORY_BY_PLAYER.get(world.player)
    if mem is None or world.step <= 0 or world.step < mem.last_step:
        mem = V8Memory(player=world.player)
        _V8_MEMORY_BY_PLAYER[world.player] = mem
    return mem


def _select_v8_mode(world, modes, mem):
    n_my = len(world.my_planets)
    n_enemy = len(world.enemy_planets)
    structural_finish = (
        n_my >= V8_FINISH_PLANET_THRESHOLD
        and n_enemy <= V8_FINISH_ENEMY_PLANET_THRESHOLD
        and world.step >= 55
    )
    under_conversion_curve = (
        (world.step <= 60 and n_my < V8_CONVERSION_T60_TARGET)
        or (world.step <= 100 and n_my < V8_CONVERSION_T100_TARGET)
    )

    if world.threatened_candidates and modes.get("is_behind"):
        next_mode = "defend"
    elif modes.get("is_finishing") or structural_finish:
        next_mode = "finish"
    elif under_conversion_curve or modes.get("is_conversion_push") or modes.get("is_alarm"):
        next_mode = "conversion"
    elif world.step < 30:
        next_mode = "opening"
    elif (n_my >= V8_STAGING_MIN_PLANETS
          and world.step >= V8_STAGING_MIN_STEP
          and not world.is_late):
        next_mode = "staging"
    else:
        next_mode = "balanced"

    if next_mode != mem.mode:
        mem.mode = next_mode
        mem.mode_since = world.step
    mem.last_step = world.step
    mem.last_planet_count = n_my
    mem.last_production = world.my_prod
    return next_mode


def _attach_v8_mode(world, modes):
    modes = dict(modes)
    mem = _get_v8_memory(world)
    v8_mode = _select_v8_mode(world, modes, mem)
    modes["v8_mode"] = v8_mode
    modes["v8_mode_age"] = max(0, world.step - mem.mode_since)
    modes["v8_memory"] = mem
    if v8_mode == "finish":
        modes["is_finishing"] = True
        modes["attack_margin_mult"] += 0.04
    elif v8_mode == "conversion":
        modes["is_conversion_push"] = True
    return modes
...
def _target_value(target, arrival_turns, mission, world, modes):
    v8_mode = modes.get("v8_mode", "balanced")
    ...
    if target.owner not in (-1, world.player):
        ...
        if v8_mode == "finish":
            value *= V8_FINISH_HOSTILE_BOOST
        elif v8_mode == "staging" and not modes.get("is_finishing"):
            value *= V8_STAGING_ATTACK_DAMPING

    if target.owner == -1:
        neutral_mult = TWO_PLAYER_NEUTRAL_VALUE_MULT if not world.is_four_player else 1.0
        value *= neutral_mult
        if v8_mode == "opening":
            value *= V8_OPENING_NEUTRAL_BOOST
        elif v8_mode == "conversion":
            value *= V8_CONVERSION_NEUTRAL_BOOST
...
def _preferred_send(target, base_needed, arrival_turns, src_available, world, modes):
    ...
    if modes.get("v8_mode") == "finish" and target.owner not in (-1, world.player):
        margin += V8_FINISH_SEND_BONUS
...
def _execute_v8_transfers(world, modes, atk_left, append_move, busy_sources=None):
    """V8.1 supplemental transfer pass.

    Conservative gating after the first benchmark regressed:
      - finish mode only (V7 REAR staging covers other phases);
      - hard cap V8_TRANSFER_MAX_MOVES (default 2);
      - rear must be both deep-back AND fat;
      - skip any source that already issued a move this turn (busy_sources);
      - shorter travel cap to avoid long-haul detours.
    """
    if not V8_TRANSFER_ENABLED or world.is_late or len(world.my_planets) < 2:
        return 0
    v8_mode = modes.get("v8_mode", "balanced")
    if V8_TRANSFER_FINISH_ONLY and v8_mode != "finish":
        return 0
    targets = _frontier_targets(world)
    if not targets:
        return 0
    ...
    safe_fronts = [
        ...
        if p.id not in world.doomed_candidates and p.id not in world.threatened_candidates
    ]
    if len(safe_fronts) < 2:
        return 0

    frontier_distance = _frontier_distance_map(world)
    front_anchor = min(safe_fronts, key=lambda p: frontier_distance[p.id])
    mem = modes.get("v8_memory")
    if mem is not None:
        mem.front_anchor_id = front_anchor.id

    ordered_distances = sorted(frontier_distance[p.id] for p in safe_fronts)
    median_distance = ordered_distances[len(ordered_distances) // 2]
    base_ratio = V8_TRANSFER_RATIO_FINISH
    busy = busy_sources or set()

    transfers = 0
    for rear in sorted(safe_fronts, key=lambda p: (-frontier_distance[p.id], -int(p.ships))):
        if transfers >= V8_TRANSFER_MAX_MOVES:
            break
        if rear.id == front_anchor.id or rear.id in busy:
            continue
        avail = atk_left(rear.id)
        if avail < V8_TRANSFER_MIN_SHIPS:
            continue

        hoard_limit = max(
            V8_TRANSFER_MIN_SHIPS,
            int(rear.production * V8_TRANSFER_HOARD_PROD_RATIO),
            int(rear.production * V8_TRANSFER_MIN_PROD_RATIO + 2),
        )
        is_back = frontier_distance[rear.id] >= max(
            median_distance,
            frontier_distance[front_anchor.id] * V8_TRANSFER_BACK_RATIO,
        )
        is_fat = avail > hoard_limit
        # V8.1: strict — must be deep-rear AND fat. No more loose gates.
        if not (is_back and is_fat):
            continue

        stage_candidates = [
            p for p in safe_fronts
            if p.id != rear.id
            and frontier_distance[p.id] < frontier_distance[rear.id] * V8_TRANSFER_STAGE_PROGRESS
        ]
        if stage_candidates:
            front = min(stage_candidates, key=lambda p: (_dist(rear.x, rear.y, p.x, p.y), frontier_distance[p.id]))
        else:
            front = front_anchor
        if front.id == rear.id:
            continue

        send = max(int(avail * base_ratio), int((avail - hoard_limit) * 0.85))
        send = min(send, avail)
        if send < V8_TRANSFER_MIN_SHIPS:
            continue

        aim = world.plan_shot(rear.id, front.id, send)
        if aim is None or aim[1] > V8_TRANSFER_MAX_TRAVEL_TURNS:
            continue
        actual = append_move(rear.id, aim[0], send)
        if actual >= V8_TRANSFER_MIN_SHIPS:
            transfers += 1
    return transfers
...
def plan_moves(world):
    modes = _attach_v8_mode(world, _build_modes(world))
    ...
    # V8.1: transfer pass moved to AFTER follow-up + V7 staging passes
    # (see end of plan_moves) so it does not steal capture/follow-up budget.
    ...
    # V8.1 supplemental transfer pass — finish mode only, runs last so it
    # cannot displace captures, V7 rear staging, or anti-hoarding. Skips any
    # source already used this turn.
    _busy_sources = {m[0] for m in moves}
    _execute_v8_transfers(world, modes, atk_left, append_move, busy_sources=_busy_sources)
...
def _load_default_training_checkpoint():
    ...
    try:
        ...
        print(
            f"[bot_v8] loaded V7 53% checkpoint baseline from { _DEFAULT_TRAINING_CHECKPOINT.name } "

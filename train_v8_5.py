#!/usr/bin/env python3
"""Long-run ES trainer for the bot_v8_5 candidate-plan ranker.

Optimizes only the lightweight V8.5 ranker/value weights — the planner grammar
...
def _migrate_flat_params(bot, params: np.ndarray) -> np.ndarray:
    """Pad old ranker checkpoints when bot_v8_5 adds plan rows or candidate features."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train bot_v8_5 ranker with antithetic ES.")
    ...
    parser.add_argument("--checkpoint", default="evaluations/v8_5_ranker_train_latest.npz")
    parser.add_argument("--best-checkpoint", default="evaluations/v8_5_ranker_train_best.npz")
    parser.add_argument("--export-bot-checkpoint", default=None,
                        help="Defaults to bot_v8_5.DEFAULT_CHECKPOINT.")
    ...
    parser.add_argument("--log-jsonl", default="evaluations/v8_5_train.jsonl")
    ...
    bot_v8_5, run_match, ZOO_main = _silent_import_bot()
    from opponents import training_pool

    if args.export_bot_checkpoint is None:
        args.export_bot_checkpoint = str(bot_v8_5.DEFAULT_CHECKPOINT)
            ...
            if generation == 1 or generation % args.eval_every == 0:
                ...
                if (gain >= args.min_improvement and non_regressing and median_ok) or \
                   (stable_gain >= args.min_improvement * 0.8 and non_regressing and eval_floor >= active_floor_baseline - 0.02):
                    ...
                    _export_bot_checkpoint(bot_v8_5, params, args.export_bot_checkpoint, {
    ...
    if best_path.exists() and best_score >= 0:
        try:
            ...
            export_params = _migrate_flat_params(bot_v8_5, export_params)
        except Exception:
            export_params = params
            export_generation = generation
    _export_bot_checkpoint(bot_v8_5, export_params, args.export_bot_checkpoint, {
        "generation": generation,

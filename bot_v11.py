"""Orbit Wars V11 — V7 tactical core + top1-style staging and task search."""
...
# --- V11 task/search layer ---
# V11 keeps the V7 world model and target scorer, but adds a top1-inspired
# supply chain: frequent friendly transfers, fewer redundant escalations, and
# stronger state-dependent finishing.
V11_ENABLED = True
V11_SEARCH_ENABLED = True
V11_SEARCH_PLY = 4
V11_MAX_SEARCH_CANDIDATES = 10
V11_SOFT_TURN_BUDGET_SEC = 0.82
V11_HARD_TURN_BUDGET_SEC = 1.35
V11_FALLBACK_OVERAGE_TIME = 2.5
V11_TRANSFER_TARGET_RATIO = 0.35
V11_TRANSFER_SEND_RATIO_4P = 0.82
V11_TRANSFER_SEND_RATIO_2P = 0.64
V11_TRANSFER_MIN_SHIPS = 6
V11_HUB_HOARD_PROD_RATIO = 5
V11_FRONT_HOARD_PROD_RATIO = 12
V11_ALREADY_EN_ROUTE_SKIP_RATIO = 0.55
V11_ALREADY_EN_ROUTE_SKIP_ENABLED = False
V11_GARRISON_ABS_MIN = 5
V11_GARRISON_BACK_PROD = 2.0
V11_GARRISON_MID_PROD = 4.0
V11_GARRISON_FRONT_PROD = 7.0
V11_FINISH_PLANET_THRESHOLD_4P = 4
V11_FINISH_PLANET_THRESHOLD_2P = 5
V11_FINISH_MIN_PLANETS = 10
V11_OPENING_ALL_NEUTRALS = True
V11_OPENING_PUNCH_TURNS = 55
V11_OPENING_MIN_CAPTURE_SEND_2P = 14
V11_OPENING_MIN_CAPTURE_SEND_4P = 16
V11_MIDGAME_MIN_CAPTURE_SEND_4P = 38
V11_CAPTURE_GARRISON_MARGIN = 0.35
V11_CAPTURE_TARGET_SHIP_MARGIN = 0.18
V11_MIDGAME_TARGET_SHIP_MARGIN_4P = 0.40
V11_CLOSE_NEUTRAL_DIST_4P = 44.0
V11_LONG_OPENING_ATTACK_DIST_4P = 58.0
V11_AGGRESSIVE_MAX_OPENING_TRANSFERS_4P = 3
V11_STRICT_FOCUS_ENABLED = True
V11_TASK_MEMORY_ENABLED = True
V11_TASK_MEMORY_MAX = 64
...
def _v11_my_ships_en_route_to(world, target_id):
    return sum(
        int(ships)
        for _eta, owner, ships in world.arrivals_by_planet.get(target_id, [])
        if owner == world.player and ships > 0
    )


def _v11_nearest_enemy_dist(world, planet):
    return min(
        (_dist(planet.x, planet.y, enemy.x, enemy.y) for enemy in world.enemy_planets),
        default=10 ** 9,
    )


def _v11_garrison_floor(world, planet):
    if planet.owner != world.player:
        return 0
    dist = _v11_nearest_enemy_dist(world, planet)
    if world.step < 28:
        if world.is_four_player and dist < 16:
            return min(int(planet.ships), max(2, int(math.ceil(planet.production * 1.5))))
        return 0
    if dist < 20:
        prod_mult = V11_GARRISON_FRONT_PROD
    elif dist < 35:
        prod_mult = V11_GARRISON_MID_PROD
    else:
        prod_mult = V11_GARRISON_BACK_PROD
    floor = int(math.ceil(float(planet.production) * prod_mult))
    if world.step < V11_OPENING_PUNCH_TURNS:
        floor = int(math.ceil(floor * (0.25 if world.is_four_player else 0.35)))
    elif world.is_four_player and world.step < 120:
        floor = int(math.ceil(floor * 0.90))
    if world.is_late:
        floor = int(math.ceil(floor * 0.70))
    abs_min = 2 if world.step < V11_OPENING_PUNCH_TURNS else V11_GARRISON_ABS_MIN
    return min(int(planet.ships), max(abs_min, floor))


_V11_TASK_MEMORY = {}


def _v11_memory_key(world):
    init_ids = tuple(sorted(world.initial_by_id.keys())[:16])
    return (world.player, len(world.planets), init_ids)


def _v11_memory(world):
    if not V11_TASK_MEMORY_ENABLED:
        return {}
    if len(_V11_TASK_MEMORY) > V11_TASK_MEMORY_MAX:
        _V11_TASK_MEMORY.clear()
    key = _v11_memory_key(world)
    mem = _V11_TASK_MEMORY.setdefault(key, {})
    if world.step <= 1:
        mem.clear()
    return mem


def _v11_focus_enemy(world):
    if not world.enemy_planets:
        return None
    mem = _v11_memory(world)
    focus = mem.get("focus_enemy_id")
    if focus is not None and any(p.owner == focus for p in world.enemy_planets):
        return focus
    focus = world.weakest_enemy_id
    mem["focus_enemy_id"] = focus
    return focus


def _v11_capture_floor(world, target, turns, needed, src_available):
    if not V11_ENABLED or target.owner == world.player:
        return needed
    if world.step < V11_OPENING_PUNCH_TURNS:
        floor = V11_OPENING_MIN_CAPTURE_SEND_4P if world.is_four_player else V11_OPENING_MIN_CAPTURE_SEND_2P
        garrison = int(math.ceil(target.production * turns * V11_CAPTURE_GARRISON_MARGIN
                                 + max(0.0, target.ships) * V11_CAPTURE_TARGET_SHIP_MARGIN))
        return min(src_available, max(int(needed), int(floor), int(needed + garrison)))
    if world.is_four_player and world.step < 130:
        garrison = int(math.ceil(target.production * 2.0 + max(0.0, target.ships) * V11_MIDGAME_TARGET_SHIP_MARGIN_4P))
        return min(src_available, max(int(needed), V11_MIDGAME_MIN_CAPTURE_SEND_4P, int(needed + garrison)))
    return int(needed)


def _v11_target_distance_from_sources(world, target):
    return min(
        (_dist(src.x, src.y, target.x, target.y) for src in world.my_planets),
        default=10 ** 9,
    )


def _v11_has_affordable_close_neutral(world):
    if not world.neutral_planets:
        return False
    for src in world.my_planets:
        available = world.available.get(src.id, 0)
        if available < 4:
            continue
        for target in world.neutral_planets:
            if _dist(src.x, src.y, target.x, target.y) > V11_CLOSE_NEUTRAL_DIST_4P:
                continue
            rough = world.plan_shot(src.id, target.id, max(1, min(available, int(target.ships) + 1)))
            if rough is None:
                continue
            needed = world.ships_needed_to_capture(target.id, rough[1])
            if 0 < needed <= available:
                return True
    return False


def _v11_target_allowed(world, target):
    if not V11_ENABLED or not world.is_four_player:
        return True
    if target.owner == -1:
        if world.step >= V11_OPENING_PUNCH_TURNS:
            return True
        d = _v11_target_distance_from_sources(world, target)
        return (
            d <= V11_CLOSE_NEUTRAL_DIST_4P
            or target.production >= 5
            or target.ships / max(1.0, target.production) <= 4.5
        )
    if target.owner not in (-1, world.player):
        focus = _v11_focus_enemy(world)
        if target.owner == focus:
            return True
        d = _v11_target_distance_from_sources(world, target)
        if (
            world.is_late
            or len(world.my_planets) < 5
            or d <= V11_CLOSE_NEUTRAL_DIST_4P
            or int(target.ships) <= FOUR_PLAYER_OPPORTUNISTIC_GARRISON_LIMIT
        ):
            return True
        return False
    return True
...
class WorldModel:
    ...
    def _compute_defense_buffers(self):
        ...
        for planet in self.my_planets:
            ...
            garrison_floor = _v11_garrison_floor(self, planet) if V11_ENABLED else 0
            reserve[planet.id] = min(int(planet.ships), max(exact_keep, proactive_keep, garrison_floor))
...
def _build_modes(world):
    ...
    if world.is_four_player:
        ...
        weakest_planets = sum(1 for p in world.enemy_planets if p.owner == world.weakest_enemy_id)
        is_finishing = (domination > FOUR_PLAYER_AGGRESSIVE_FINISHING_DOMINATION
                        ...
                        and world.step > 80)
        if V11_ENABLED and world.step > 70 and len(world.my_planets) >= V11_FINISH_MIN_PLANETS:
            is_finishing = is_finishing or (
                world.weakest_enemy_id is not None
                and 0 < weakest_planets <= V11_FINISH_PLANET_THRESHOLD_4P
                and world.my_total >= world.owner_strength.get(world.weakest_enemy_id, 0) * 1.05
            )
    else:
        ...
        is_finishing = (domination > FINISHING_DOMINATION
                        ...
                        and world.step > 100)
        if V11_ENABLED and world.step > 80 and len(world.my_planets) >= V11_FINISH_MIN_PLANETS:
            is_finishing = is_finishing or (
                0 < enemy_planets <= V11_FINISH_PLANET_THRESHOLD_2P
                and world.my_total >= world.enemy_total * 0.95
            )
...
def _opening_filter(target, arrival_turns, needed, src_available, world):
    if not _v11_target_allowed(world, target):
        return True
    if not world.is_opening or target.owner != -1:
        return False
    if target.id in world.comet_ids or world.is_static(target.id):
        return False
    if V11_ENABLED and world.is_four_player and world.step < V11_OPENING_PUNCH_TURNS:
        best_dist = _v11_target_distance_from_sources(world, target)
        affordable = needed <= max(src_available, int(src_available * 1.15))
        if affordable and (
            best_dist <= V11_LONG_OPENING_ATTACK_DIST_4P
            or target.production >= 5
            or target.ships / max(1.0, target.production) <= 4.5
        ):
            return False
...
def _target_value(target, arrival_turns, mission, world, modes):
    ...
    if world.is_four_player:
        if target.owner not in (-1, world.player):
            ...
            focus_enemy_id = _v11_focus_enemy(world) if V11_ENABLED else world.weakest_enemy_id
            if target.owner == focus_enemy_id:
    ...
    if world.is_four_player and target.owner not in (-1, world.player):
        focus_enemy_id = _v11_focus_enemy(world) if V11_ENABLED else world.weakest_enemy_id
        if target.owner == focus_enemy_id:
            value *= AOW_4P_WEAKEST_FOCUS_BOOST * (1.18 if V11_ENABLED else 1.0)
        elif not world.is_late:
            value *= AOW_4P_NON_WEAKEST_PENALTY * (0.55 if V11_ENABLED else 1.0)
...
def _preferred_send(target, base_needed, arrival_turns, src_available, world, modes):
    ...
    result = send + margin
    if V11_ENABLED:
        result = max(result, _v11_capture_floor(world, target, arrival_turns, base_needed, src_available))
    return min(src_available, result)
...
def _v11_frontier_targets(world):
    if world.enemy_planets:
        focus = _v11_focus_enemy(world) if world.is_four_player else None
        focused = [p for p in world.enemy_planets if focus is None or p.owner == focus]
        if focused:
            return focused
    return world.static_neutral_planets or world.neutral_planets or world.enemy_planets


def _v11_frontier_distance(world, targets):
    if not targets:
        return {p.id: 10 ** 9 for p in world.my_planets}
    return {
...
def _v11_task_search_bias(world, modes):
    if not V11_SEARCH_ENABLED:
        return {"transfer": 1.0, "strike": 1.0, "expand": 1.0, "finish": 1.0}
    my_planets = len(world.my_planets)
    enemy_planets = len(world.enemy_planets)
    neutral_planets = len(world.neutral_planets)
    pressure = len(world.threatened_candidates) + 0.5 * len(world.doomed_candidates)
    garrison_ratio = sum(p.ships for p in world.my_planets) / max(1.0, float(world.my_total))
    base = {
        "transfer": 1.0,
        "strike": 1.0,
        "expand": 1.0,
    ...
    frontier_gap = max(0.0, 13.0 - my_planets) / 13.0
    for ply in range(max(1, int(V11_SEARCH_PLY))):
        phase = min(1.0, (world.step + ply * 8) / 160.0)
        base["expand"] += (1.0 - phase) * frontier_gap * (1.75 if neutral_planets else 0.25)
        base["transfer"] += 0.42 + 0.45 * phase + max(0.0, 0.40 - garrison_ratio)
        base["strike"] += 0.30 * phase + (0.35 if my_planets >= 7 else 0.0)
        base["defense"] += 0.35 * pressure + max(0.0, 0.28 - garrison_ratio)
        if modes.get("is_finishing") or (enemy_planets and my_planets >= V11_FINISH_MIN_PLANETS):
            base["finish"] += 0.55 + 0.18 * ply
    return base


def _v11_commit_supply_chain(world, modes, planned_commitments, atk_left, append_move, *, early=False):
    if not V11_ENABLED or len(world.my_planets) <= 1 or world.is_late:
        return 0
    if early and world.is_four_player:
        return 0
    if early and world.step < 24 and len(world.my_planets) < 4:
        return 0
    if early and len(world.my_planets) < 6 and _v11_has_affordable_close_neutral(world):
        return 0
    if world.step < 8 and not early:
        return 0
    targets = _v11_frontier_targets(world)
    if not targets:
        return 0
    frontier_distance = _v11_frontier_distance(world, targets)
    safe = [p for p in world.my_planets if p.id not in world.doomed_candidates]
    if len(safe) <= 1:
        return 0
    mem = _v11_memory(world)
    remembered_anchor = mem.get("front_anchor_id")
    front_anchor = None
    if remembered_anchor is not None:
        for p in safe:
            if p.id == remembered_anchor:
                front_anchor = p
                break
    if front_anchor is None:
        front_anchor = min(safe, key=lambda p: (frontier_distance.get(p.id, 10 ** 9), -p.ships))
        mem["front_anchor_id"] = front_anchor.id
    bias = modes.get("v11_bias") or _v11_task_search_bias(world, modes)
    transfer_mult = max(0.65, min(1.45, float(bias.get("transfer", 1.0))))
    base_ratio = V11_TRANSFER_SEND_RATIO_4P if world.is_four_player else V11_TRANSFER_SEND_RATIO_2P
    if modes.get("is_finishing"):
        base_ratio = max(base_ratio, 0.78)
    send_ratio = min(0.88, base_ratio * (0.85 + 0.12 * transfer_mult))
    min_send = V11_TRANSFER_MIN_SHIPS if not early else max(6, V11_TRANSFER_MIN_SHIPS - 2)
    sent_total = 0
    max_transfers = 7 if world.is_four_player else 5
    if early:
        max_transfers = V11_AGGRESSIVE_MAX_OPENING_TRANSFERS_4P if world.is_four_player else 2
        if world.step < 32:
            max_transfers = min(max_transfers, 2)
    if modes.get("is_finishing"):
        max_transfers += 3
    transfers = 0
    ordered = sorted(
        safe,
        key=lambda p: (frontier_distance.get(p.id, 0.0), float(p.ships)),
        reverse=True,
    )
    for rear in ordered:
        ...
        if rear.id == front_anchor.id:
            continue
        if rear.id in world.threatened_candidates:
            continue
        rear_dist = frontier_distance.get(rear.id, 10 ** 9)
        front_dist = frontier_distance.get(front_anchor.id, 0.0)
        is_back = rear_dist > max(front_dist + 8.0, front_dist * 1.18)
        hoard_limit = int(max(V11_HUB_HOARD_PROD_RATIO * rear.production, V11_TRANSFER_MIN_SHIPS))
        if not is_back and atk_left(rear.id) < hoard_limit:
            continue
        candidates = [
            p for p in safe
            if p.id != rear.id
            and frontier_distance.get(p.id, 10 ** 9) < rear_dist
            and p.id not in world.doomed_candidates
        ]
        if not candidates:
            continue
        target = min(candidates, key=lambda p: (_dist(rear.x, rear.y, p.x, p.y), frontier_distance.get(p.id, 10 ** 9)))
        left = atk_left(rear.id)
        if left < min_send:
            continue
        send = int(left * send_ratio)
        if rear_dist <= front_dist + 16:
            send = min(send, max(min_send, int(left * 0.55)))
        if send < min_send:
            continue
        aim = world.plan_shot(rear.id, target.id, send)
        if aim is None or aim[1] > REAR_MAX_TRAVEL_TURNS:
            continue
        actual = append_move(rear.id, aim[0], send)
        if actual > 0:
            planned_commitments[target.id].append((aim[1], world.player, int(actual)))
            sent_total += actual
            transfers += 1
    return sent_total
...
def plan_moves(world):
    ...
    modes["v11_bias"] = _v11_task_search_bias(world, modes) if V11_ENABLED else {}
    ...
    _v11_commit_supply_chain(world, modes, planned_commitments, atk_left, append_move, early=(world.step < 45))
    ...
    for src in world.my_planets:
        ...
        for target in world.planets:
            ...
            if rough_needed <= 0:
                continue
            already = _v11_my_ships_en_route_to(world, target.id) if V11_ENABLED else 0
            if V11_ALREADY_EN_ROUTE_SKIP_ENABLED and already >= rough_needed:
                continue
            if (V11_ALREADY_EN_ROUTE_SKIP_ENABLED and target.owner not in (-1, world.player)
                    and already > rough_needed * V11_ALREADY_EN_ROUTE_SKIP_RATIO):
                continue
            ...
            if needed <= 0:
                continue
            already = _v11_my_ships_en_route_to(world, target.id) if V11_ENABLED else 0
            if V11_ALREADY_EN_ROUTE_SKIP_ENABLED and already >= needed:
                continue
            if (V11_ALREADY_EN_ROUTE_SKIP_ENABLED and target.owner not in (-1, world.player)
                    and already > needed * V11_ALREADY_EN_ROUTE_SKIP_RATIO):
                continue
            ...
            if V11_ENABLED:
                bias = modes.get("v11_bias", {})
                if target.owner == -1:
                    score *= max(0.75, min(1.45, float(bias.get("expand", 1.0))))
                elif modes.get("is_finishing"):
                    score *= max(1.0, min(1.75, float(bias.get("finish", 1.0))))
                else:
                    score *= max(0.85, min(1.35, float(bias.get("strike", 1.0))))
            option = ShotOption(score=score, src_id=src.id, target_id=target.id,
    ...
    for target_id, options in source_options_by_target.items():
        ...
        if FOUR_SOURCE_SWARM_ENABLED and hostile_target and int(target.ships) >= FOUR_SOURCE_MIN_TARGET_SHIPS and len(top) >= 4:
            limit = min(len(top), 7)
            for i in range(limit):
                for j in range(i + 1, limit):
                    for k in range(j + 1, limit):
                        for l in range(k + 1, limit):
                            ...
                            if len({o.src_id for o in quad}) < 4:
                                continue
                            if max(o.turns for o in quad) - min(o.turns for o in quad) > FOUR_SOURCE_ETA_TOLERANCE:
                                continue
                            ...
                            if need <= 0 or sum(o.send_cap for o in quad) < need:
                                continue
                            if any(
                                sum(quad[x].send_cap for x in combo) >= need
                                for combo in ((0, 1), (0, 2), (0, 3), (1, 2), (1, 3), (2, 3))
                            ):
                                continue
                            value = _target_value(target, jt, "swarm", world, modes)
                            if value <= 0:
                                continue
                            qs = _apply_score_mods(value / (need + jt * ATTACK_COST_TURN_WEIGHT + 1.0),
                                                    target, "swarm", world) * FOUR_SOURCE_PLAN_PENALTY
                            missions.append(Mission(kind="swarm", score=qs, target_id=target_id, turns=jt, options=quad))
        ...
        for src in world.my_planets:
            ...
            for target in world.planets:
                ...
                if V11_ENABLED:
                    bias = modes.get("v11_bias", {})
                    if target.owner == -1:
                        score *= max(0.75, min(1.35, float(bias.get("expand", 1.0))))
                    elif modes.get("is_finishing"):
                        score *= max(1.0, min(1.65, float(bias.get("finish", 1.0))))
    ...
    _v11_commit_supply_chain(world, modes, planned_commitments, atk_left, append_move, early=False)
...
_TUNABLE_KEYS = (
    ...
    "FOUR_PLAYER_WEAKEST_ENEMY_BOOST",
    "AOW_4P_WEAKEST_FOCUS_BOOST",
    "AOW_4P_NON_WEAKEST_PENALTY",
    "V11_TRANSFER_SEND_RATIO_4P",
    "V11_TRANSFER_MIN_SHIPS",
    "V11_HUB_HOARD_PROD_RATIO",
    "V11_GARRISON_BACK_PROD",
    "V11_GARRISON_MID_PROD",
    "V11_GARRISON_FRONT_PROD",
    "V11_FINISH_MIN_PLANETS",
    "V11_OPENING_MIN_CAPTURE_SEND_4P",
    "V11_MIDGAME_MIN_CAPTURE_SEND_4P",
    "V11_CAPTURE_GARRISON_MARGIN",
    "V11_CAPTURE_TARGET_SHIP_MARGIN",
    "V11_MIDGAME_TARGET_SHIP_MARGIN_4P",
    "V11_CLOSE_NEUTRAL_DIST_4P",
    "V11_LONG_OPENING_ATTACK_DIST_4P",
    "V11_AGGRESSIVE_MAX_OPENING_TRANSFERS_4P",
)
_DEFAULT_PARAMS = {k: globals()[k] for k in _TUNABLE_KEYS if k in globals()}
_BASE_HEURISTIC_DEFAULTS = dict(_DEFAULT_PARAMS)
_DEFAULT_TRAINING_CHECKPOINT = Path(__file__).resolve().parent / "evaluations" / "scorer_v11_kaggle.npz"
_FALLBACK_TRAINING_CHECKPOINT = Path(__file__).resolve().parent / "evaluations" / "scorer_v7_kaggle.npz"
_DEFAULT_CHECKPOINT_HEURISTIC_SPECS = (
    ...
    ("FOUR_PLAYER_WEAKEST_ENEMY_BOOST", 1.20, 0.25),
    ("AOW_4P_WEAKEST_FOCUS_BOOST", 2.10, 0.45),
    ("AOW_4P_NON_WEAKEST_PENALTY", 0.62, 0.20),
    ("V11_TRANSFER_SEND_RATIO_4P", 0.82, 0.12),
    ("V11_TRANSFER_MIN_SHIPS", 6.0, 3.0),
    ("V11_HUB_HOARD_PROD_RATIO", 5.0, 2.0),
    ("V11_GARRISON_BACK_PROD", 2.0, 1.0),
    ("V11_GARRISON_MID_PROD", 4.0, 1.5),
    ("V11_GARRISON_FRONT_PROD", 7.0, 2.0),
    ("V11_FINISH_MIN_PLANETS", 10.0, 3.0),
    ("V11_OPENING_MIN_CAPTURE_SEND_4P", 16.0, 5.0),
    ("V11_MIDGAME_MIN_CAPTURE_SEND_4P", 28.0, 8.0),
    ("V11_CAPTURE_GARRISON_MARGIN", 0.35, 0.16),
    ("V11_CAPTURE_TARGET_SHIP_MARGIN", 0.18, 0.10),
    ("V11_MIDGAME_TARGET_SHIP_MARGIN_4P", 0.40, 0.18),
    ("V11_CLOSE_NEUTRAL_DIST_4P", 44.0, 8.0),
    ("V11_LONG_OPENING_ATTACK_DIST_4P", 58.0, 10.0),
    ("V11_AGGRESSIVE_MAX_OPENING_TRANSFERS_4P", 3.0, 2.0),
)
...
def _load_default_training_checkpoint():
    ...
    ckpt_path = _DEFAULT_TRAINING_CHECKPOINT
    if not ckpt_path.exists():
        ckpt_path = _FALLBACK_TRAINING_CHECKPOINT
    if not ckpt_path.exists():
        return False
    try:
        ...
        label = "V11" if ckpt_path == _DEFAULT_TRAINING_CHECKPOINT else "V7"
        print(
            f"[bot_v11] loaded {label} checkpoint from { ckpt_path.name } "
...
def agent(obs, config=None):
    start = time.time()
    try:
        remaining = float(_get(obs, "remainingOverageTime", 60.0) or 60.0)
        world = _build_world(obs)
        if not world.my_planets:
            return []
        if remaining <= V11_FALLBACK_OVERAGE_TIME:
            return _fallback_plan_moves(world)
...
def _fallback_plan_moves(world):
    global V11_ENABLED, V11_SEARCH_ENABLED
    old_enabled = V11_ENABLED
    old_search = V11_SEARCH_ENABLED
    V11_ENABLED = False
    V11_SEARCH_ENABLED = False
    try:
        return plan_moves(world)
    finally:
        V11_ENABLED = old_enabled
        V11_SEARCH_ENABLED = old_search

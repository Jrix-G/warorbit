...
_CHECKPOINT_53_SCORER_WEIGHTS = np.array([
    0.3662528395652771,
    -0.4687938392162323,
    0.16412760317325592,
    0.18628084659576416,
    ...
    -0.3227209448814392,
    0.7785797715187073,
    0.001532945898361504,
    0.008243115618824959,
], dtype=np.float64)
_CHECKPOINT_53_HEURISTIC_PARAMS = {
    "HOSTILE_MARGIN_BASE": 3.331252411007881,
    "HOSTILE_MARGIN_CAP": 12.728436708450317,
    "HOSTILE_MARGIN_PROD_WEIGHT": 1.6392886638641357,
    "NEUTRAL_MARGIN_BASE": 2.2531009912490845,
    "TWO_PLAYER_HOSTILE_AGGRESSION_BOOST": 1.4868186235427858,
    "ATTACK_COST_TURN_WEIGHT": 0.6561033129692078,
    "STATIC_TARGET_MARGIN": 3.4300166815519333,
}
...
def _decode_training_checkpoint_params(params):
    ...
    if params.size < MISSION_FEATURE_DIM:
        return None
    return (
        _CHECKPOINT_53_SCORER_WEIGHTS.copy(),
        dict(_CHECKPOINT_53_HEURISTIC_PARAMS),
    )


def _load_default_training_checkpoint():
    try:
        decoded = _decode_training_checkpoint_params(_CHECKPOINT_53_SCORER_WEIGHTS)
        if decoded is None:
            return False
        ...
        print("[submission] loaded checkpoint 53% from embedded snapshot (gen=273, wr=0.533)", flush=True)
        return True
    except Exception:
        return False

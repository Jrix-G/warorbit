"""Collect V21 counterfactual-oracle samples from simulated games."""
...
class CollectConfig:
    ...
    steps: int = 80
    stride: int = 10
    seed_offset: int = 41_000_000
    horizon: int = 8
    top_k: int = 8
    policy: str = "pass"
    policy_budget: float = 0.02
    max_samples: int = 64
    oracle_mode: str = "atomic"
    max_combo: int = 4
    beam_width: int = 32
...
def collect_oracle_samples(cfg: CollectConfig) -> list[dict[str, Any]]:
    ...
    if cfg.oracle_mode not in ("atomic", "combo"):
        raise ValueError("oracle_mode must be atomic or combo")
            ...
            for player in range(int(cfg.n_players)):
                ...
                if step % int(cfg.stride) == 0 and len(samples) < int(cfg.max_samples):
                    try:
                        ...
                        if str(cfg.oracle_mode) == "combo":
                            sample = v21_oracle.combo_oracle_sample_from_state(
                                fs,
                                player,
                                episode_id=f"oracle-{seed}-{player}",
                                source=f"combo_oracle_{cfg.policy}",
                            ...
                            sample = v21_oracle.oracle_sample_from_state(
                                fs,
                                player,
                                episode_id=f"oracle-{seed}-{player}",
                                source=f"oracle_{cfg.policy}",
                                horizon=int(cfg.horizon),
                                top_k=int(cfg.top_k),
                            )
                        ...
                        pass
                actions.append(_policy_action(str(cfg.policy), obs, agent_cfg, float(cfg.policy_budget)))
...
def _policy_action(policy: str, obs: dict[str, Any], config: AgentConfig, budget: float) -> list:
    ...
    if policy == "v15":
        try:
            move = v15_search.search(obs, config, time_budget=budget, horizon=10)
...
def write_samples(path: str, samples: list[dict[str, Any]]) -> int:
    return v21_dataset.write_jsonl(path, samples)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect V21 oracle samples from simulated games")
    parser.add_argument("--out", required=True)
    parser.add_argument("--n-players", type=int, default=4)
    parser.add_argument("--games", type=int, default=1)
    parser.add_argument("--steps", type=int, default=80)
    parser.add_argument("--stride", type=int, default=10)
    parser.add_argument("--seed-offset", type=int, default=41_000_000)
    parser.add_argument("--horizon", type=int, default=8)
    parser.add_argument("--top-k", type=int, default=8)
    parser.add_argument("--policy", choices=["pass", "v15"], default="pass")
    parser.add_argument("--policy-budget", type=float, default=0.02)
    parser.add_argument("--max-samples", type=int, default=64)
    parser.add_argument("--oracle-mode", choices=["atomic", "combo"], default="atomic")
...
def main() -> None:
    ...
    samples = collect_oracle_samples(cfg)
    written = write_samples(args.out, samples)

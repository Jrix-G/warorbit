#!/usr/bin/env python3
"""Reproducible local benchmark for bot_v8_5.
...
def _silent_imports():
    ...
    return bot_v8_5, bot_v7, run_match, ZOO
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark bot_v8_5 against local opponents.")
    ...
    bot_v8_5_main, bot_v7_main, run_match, ZOO_main = _silent_imports()
    from opponents import training_pool

    opponents = training_pool(args.pool_limit) if args.pool_limit > 0 else list(args.opponents)
    missing = [name for name in opponents if name not in ZOO_main and name not in {"bot_v7", "bot_v8_5"}]
    if missing:
    ...
    print(
        f"V8.5 benchmark | mode={args.mode} games={len(tasks)} "

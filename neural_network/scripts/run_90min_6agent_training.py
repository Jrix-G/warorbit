...
def _prepare_config(cfg: dict, duration_minutes: float, workers: int, eval_episodes: int) -> dict:
    ...
    cfg["worker_train_steps"] = max(2, int(cfg.get("worker_train_steps", 4)))
    ...
    for key in ("checkpoint_dir", "log_dir", "candidate_checkpoint", "best_checkpoint", "latest_checkpoint", "export_path"):
        if key in cfg:

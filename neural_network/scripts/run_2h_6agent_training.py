...
def _run_tag() -> str:
    return datetime.now(timezone.utc).strftime("run_2h_%Y%m%d_%H%M%S")
...
def _prepare_config(cfg: dict, duration_minutes: float, workers: int, eval_episodes: int, run_name: str) -> dict:
    ...
    cfg["worker_train_steps"] = max(8, min(64, int(cfg.get("worker_train_steps", 8))))
    cfg["imitation_warmstart_steps"] = max(32, min(64, int(cfg.get("imitation_warmstart_steps", 32))))
    cfg["eval_episodes"] = max(8, min(16, int(eval_episodes)))
    cfg["candidate_eval_episodes"] = max(8, min(cfg["eval_episodes"], int(cfg.get("candidate_eval_episodes", 8))))
    cfg["promotion_eval_episodes"] = max(12, min(24, max(cfg["eval_episodes"], int(cfg.get("promotion_eval_episodes", 12)))))
    cfg["promotion_min_remaining_minutes"] = max(8.0, float(cfg.get("promotion_min_remaining_minutes", 10.0)))
    cfg["min_generation_remaining_minutes"] = max(0.75, min(4.0, float(duration_minutes) * 0.04))
    ...
    cfg["notebook_pool_limit"] = 12
    cfg["notebook_pool_limit_max"] = 12
    ...
    cfg["max_turns"] = min(int(cfg.get("max_turns", 100)), 70)
    ...
    cfg["dense_ship_share_coef"] = 0.15
    cfg["dense_score_coef"] = 0.10
    cfg["dense_survival_coef"] = 0.08
    cfg["dense_reward_clip"] = 0.40
    ...
    cfg["opponent_curriculum_tiers"] = [
        ...
        },
        {
            ...
            "min_generations": 1,
            "advance_score": 0.18,
            "advance_winrate": 0.02,
            "advance_rank_mean": 2.95,
            "advance_do_nothing_rate": 0.58,
    ...
    cfg["temperature_start"] = max(float(cfg.get("temperature_start", 1.20)), 1.25)
    cfg["temperature_end"] = min(float(cfg.get("temperature_end", 0.20)), 0.15)
    ...
    cfg["promotion_margin"] = max(0.01, float(cfg.get("promotion_margin", 0.02)))
    cfg["promotion_min_winrate"] = min(0.05, float(cfg.get("promotion_min_winrate", 0.05)))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a fresh 2-hour 4p population training job.")
    ...
    result = run_population_4p_training(cfg, resume=bool(args.resume))

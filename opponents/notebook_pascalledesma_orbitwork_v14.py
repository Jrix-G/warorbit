...
def _eval_moves(world, moves, sim_turns=20):
    """Score a set of moves by simulating forward sim_turns and evaluating position."""
    import copy
    # Deep copy world state for simulation
    w = copy.deepcopy(world)
    # Apply moves (simplified: add ships to target planets)
    for src_id, angle, ships in moves:
        pass  # moves format: [src_id, angle, send] - simplified simulation
    # Simple simulation: just return a heuristic based on moves properties
    total_ships_sent = sum(int(m[2]) for m in moves)
    return total_ships_sent
...
def agent(obs, config=None):
    ...
    # Multi-candidate: run default + conservative, pick by move quality
    base_moves = plan_moves(world, deadline=deadline)
    
    # Evaluate base moves: count attack vs reinforce ratio
    def move_quality(moves):
        if not moves:
            return 0.0
        attack_ships = sum(int(m[2]) for m in moves if int(m[2]) > 10)
        total_ships = sum(int(m[2]) for m in moves)
        return float(attack_ships) / max(1, total_ships)
    
    base_score = move_quality(base_moves)
    
    # Try conservative constants (lower aggression)
    orig = (HOSTILE_TARGET_VALUE_MULT, ELIMINATION_BONUS, STATIC_HOSTILE_VALUE_MULT, OPENING_HOSTILE_TARGET_VALUE_MULT)
    HOSTILE_TARGET_VALUE_MULT = 1.5
    ELIMINATION_BONUS = 12.0
    STATIC_HOSTILE_VALUE_MULT = 1.3
    OPENING_HOSTILE_TARGET_VALUE_MULT = 1.2
    try:
        cons_moves = plan_moves(world, deadline=deadline)
        cons_score = move_quality(cons_moves)
    finally:
        HOSTILE_TARGET_VALUE_MULT, ELIMINATION_BONUS, STATIC_HOSTILE_VALUE_MULT, OPENING_HOSTILE_TARGET_VALUE_MULT = orig
    
    # Return the moves from the strategy with higher attack ratio
    return base_moves if base_score >= cons_score else cons_moves


__all__ = ["agent", "build_world", "_eval_position"]

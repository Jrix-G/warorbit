...
RUN_NAME="${RUN_NAME:-v9_top1_punch_trainonly_30m_12w}"
...
python3 run_v9.py \
  ...
  --eval-games 0 \
  --benchmark-games 0 \
  --min-promotion-benchmark-games 0 \
  --benchmark-progress-every 0 \
  --eval-every 1 \
  --benchmark-every 0 \
  --skip-eval \
  --max-steps 120 \
  ...
  --eval-four-player-ratio 0.85 \
  --benchmark-four-player-ratio 0.85 \
  --train-only \
